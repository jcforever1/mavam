...
def _canonical_signal_ref(signal) -> str:
...
def _validate_kronos(
    ...
    if not adj.kronos.required:
        if kronos is None:
            return reasons  # not required, skip
        # Optional Kronos: still apply min_confidence and version if set
        if kronos.confidence < adj.kronos.min_confidence:
            reasons.append("kronos_low_confidence")
        if (adj.kronos.allowed_model_versions
                and kronos.model_version not in adj.kronos.allowed_model_versions):
            reasons.append("kronos_version_not_allowed")
        return reasons

    # Kronos is required
    if kronos is None:
        reasons.append("kronos_required_but_missing")
        return reasons
...
def _evaluate_merge_rules(
    ...
    Returns (emit, size_multiplier, reason) or None if no rule matches.
    """
    ...
    for rule in adj.merge_rules:
        try:
            ...
            # A malformed rule shouldn't poison the whole merger; skip it
            continue
        if matched:
...
def _fallback_action(adj: Adjudicator, book, kronos) -> tuple[Action, str]:
    """Pick the appropriate fallback action based on what's missing."""
    if book is None:
...
def merge(
    ...
    *,
    ...
    # 1. Validate inputs
    book_reasons = _validate_book(adj, book) if book is not None else (
        ["book_required_but_missing"] if adj.book.required else []
    )
    kronos_reasons = _validate_kronos(adj, kronos)
    validation_reasons = book_reasons + kronos_reasons

    # 2. If validation fails, fall back
    if validation_reasons:
        ...
        return TradeSignal(
            ...
            reason=f"validation_failed: {'; '.join(validation_reasons)}; "
                   f"fallback={reason}",
            book_signal_ref=_canonical_signal_ref(book),
            kronos_signal_ref=_canonical_signal_ref(kronos),
            ...
            is_decide_no_reasons=validation_reasons,
        )

    # 3. If book is None but not required (validation passed), HOLD
    if book is None:
        return TradeSignal(
            action=Action.HOLD,
            is_decide_no=True,
    ...
    # 5. Apply risk parameters
    qty = 0
    if action in (Action.BUY, Action.SELL) and entry_price is not None:
        # Position size = max_position_pct * size_multiplier of nominal
        # The bot is responsible for translating this into actual qty
        # given the account size; the engine emits a multiplier + price.
        # For v1, we just emit a small "suggested qty" of 1 share as a
        # marker. The bot replaces this with real sizing.
    ...
    return TradeSignal(
        ...
        decide_no_reasons=[] if not is_decide_no else ["hold_emitted_by_policy"],
        symbol=symbol,
        qty=qty,
        order_type="limit",
